...
```bash
npx @ark-preset/cli@latest add date-picker
```
...
> **Dependencies:** This component imports shared recipes or sub-components from other packages. Make sure the following are also installed: [button](/docs/components/button), [label](/docs/components/label).
