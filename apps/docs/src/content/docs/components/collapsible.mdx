...
<a href="https://ark-ui.com/docs/components/collapsible" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 mt-4 text-sm text-muted-foreground hover:text-foreground transition-colors">
...
```tsx
import { Collapsible, CollapsibleTrigger, CollapsibleContent, CollapsibleIndicator } from "~/components/collapsible"
...
<Collapsible>
  ...
  <CollapsibleContent>
    <div class="pt-4 text-sm">
      The indicator automatically rotates when expanded.
    </div>
  </CollapsibleContent>
</Collapsible>
```
