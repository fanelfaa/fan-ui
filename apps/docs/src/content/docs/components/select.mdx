...
<a href="https://ark-ui.com/docs/components/select" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 mt-4 text-sm text-muted-foreground hover:text-foreground transition-colors">
...
```tsx
import { Select, SelectLabel, SelectControl, SelectTrigger, SelectValueText, SelectContent, createListCollection } from "~/components/select"
...
export { SelectRoot as Select, SelectLabel, SelectControl, SelectTrigger, SelectValueText, SelectContent, SelectItem, SelectItemText, SelectItemIndicator, selectVariants }
```
</div>
