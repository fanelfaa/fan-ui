...
/**
 * usage: npx @ark-preset/cli add {component_name}
 *
...
function findPackageRoot(): string {
  ...
  console.error(
    "❌ component-manifest.json not found. Run the build script first:\n" +
      "  moon run @ark-preset/cli:generate-manifest",
  );
  process.exit(1);
}
  ...
  if (!fs.existsSync(MANIFEST_PATH)) {
    console.error(
      `❌ component-manifest.json not found at ${MANIFEST_PATH}\n` +
        "  Run the build script first: moon run @ark-preset/cli:generate-manifest\n" +
